...
const WorkerDashboard = () => {
  ...
  const initAudio = useCallback(() => {
    ...
    audioRef.current.addEventListener('error', () => (audioRef.current.src = '/noti.mp3'));
  }, []);
  ...
  const testSound = useCallback(() => {
    ...
    audioRef.current.currentTime = 0;
    audioRef.current.play().catch(() => alert('⚠️ Unable to play sound'));
  }, [initAudio]);
  ...
  const toggleSound = useCallback(async () => {
    if (!soundEnabled) {
      ...
      try {
        ...
        alert('⚠️ Could not enable sound.');
      }
    } else {
      audioRef.current?.pause();
      audioRef.current.currentTime = 0;
  ...
  useEffect(() => {
    initAudio();
    setSoundEnabled(localStorage.getItem('workerSoundEnabled') === 'true');
  }, [initAudio]);
  ...
  const handleCompleteBooking = useCallback(async (bookingId) => {
    if (!window.confirm('Have you completed this service?')) return;
    setActionLoading(bookingId);
    try {
      await axios.put(
  ...
  useEffect(() => {
    ...
    newSocket.on('newBooking', (data) => {
      ...
      autoRejectTimerRef.current = setTimeout(() => {
        if (data.booking?._id) updateBookingStatus(data.booking._id, 'reject');
      }, 30000);
  ...
  }, [user?._id, soundEnabled, playNotificationSound, updateBookingStatus, fetchBookings]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pendingEarnings: bookings.filter(b => b.bookingStatus === 'accepted').reduce((sum, b) => sum + (b.workerAmount || 0), 0),
    };
  }, [bookings]);

  const weeklyData = useMemo(() => {
    const data = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => ({ day, earnings: 0, bookings: 0 }));
    ...
    filtered.forEach(booking => {
      ...
      if (data[idx]) {
        data[idx].earnings += booking.workerAmount || 0;
        data[idx].bookings += 1;
      }
    });
    return data;
  ...
  return (
    <div className="min-h-screen bg-white pb-20 md:pb-0">
          ...
          <div className="bg-white border border-gray-200 rounded-b-2xl overflow-hidden">
            <div className="bg-black px-4 py-3">
              <div className="flex justify-between items-center">
                ...
                <button onClick={closeNotification} className="text-white/80 hover:text-white text-xl">✕</button>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-gray-50 rounded-lg p-2 border border-gray-100">
                  ...
                  <p className="text-sm font-semibold text-black">{new Date(notification.booking.date).toLocaleDateString()}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-2 border border-gray-100">
              ...
              <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <button onClick={() => openInMaps(notification.booking.location)} 
              ...
              <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-black">₹{notification.booking.workerAmount}</span>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => updateBookingStatus(notification.booking._id, 'accept')} 
                  disabled={actionLoading === notification.booking._id}
                  className="flex-1 py-3 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition active:scale-95">
                  ✓ ACCEPT
                </button>
                <button onClick={() => updateBookingStatus(notification.booking._id, 'reject')} 
                  disabled={actionLoading === notification.booking._id}
                  className="flex-1 py-3 bg-gray-200 text-black rounded-xl font-bold hover:bg-gray-300 transition active:scale-95">
                  ✗ REJECT
                </button>
              </div>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white shadow-2xl border-t border-gray-100 z-50 md:hidden">
        <div className="flex justify-around items-center py-2">
          <div className="flex flex-col items-center">
            <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
            <span className="text-xs text-gray-500 mt-1">{isConnected ? 'Online' : 'Offline'}</span>
          </div>
          ...
          {!soundEnabled ? (
            <button onClick={toggleSound} className="flex flex-col items-center p-2">
              ...
              <span className="text-xs text-black mt-1">Enable</span>
            </button>
          ) : (
            <button onClick={toggleSound} className="flex flex-col items-center p-2">
      ...
      {/* Desktop Navbar */}
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-40 hidden md:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={testSound} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition">
                Test Sound
              </button>
              
              {!soundEnabled ? (
                <button onClick={toggleSound} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition">
                  Enable Sound
                </button>
              ) : (
                <button onClick={toggleSound} className="px-4 py-2 bg-gray-200 text-black rounded-lg hover:bg-gray-300 transition">
                  Disable Sound
                </button>
              )}
              ...
              <button onClick={handleLogout} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                Logout
              </button>
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 py-3 px-4 space-y-2 bg-white">
            ...
            <div className="pt-2 border-t border-gray-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700">{isConnected ? 'Online' : 'Offline'}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {!soundEnabled && (
        <div className="bg-gray-50 border-l-4 border-black p-3 mx-3 mt-3 rounded-lg">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <p className="text-xs text-gray-700 flex-1">Click <strong>"Test Sound"</strong> first, then <strong>"Enable"</strong> to get audio alerts</p>
            <div className="flex gap-2">
              <button onClick={testSound} className="px-3 py-1 bg-black text-white rounded-lg text-xs font-semibold">
                Test
              </button>
              <button onClick={toggleSound} className="px-3 py-1 bg-black text-white rounded-lg text-xs font-semibold">
                Enable
              </button>
            </div>
          </div>
      ...
      {soundEnabled && (
        <div className="bg-gray-50 border-l-4 border-green-500 p-2 mx-3 mt-3 rounded-lg">
          <div className="flex items-center gap-2">
            <span className="text-lg">🔊</span>
            <p className="text-xs text-gray-700">Sound notifications are <strong>ON</strong></p>
            <button onClick={testSound} className="ml-auto px-2 py-1 bg-black text-white rounded text-xs">
              Test
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
        {/* Welcome Section */}
        <div className="mb-6">
          <h1 className="text-xl md:text-3xl font-bold text-black">Welcome back, {user?.name?.split(' ')[0]}! 👋</h1>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-3 md:p-6 shadow-sm hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg md:text-3xl font-bold text-black">₹{stats.totalEarnings.toFixed(2)}</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-4 md:p-6 shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-base md:text-lg font-bold text-black">Weekly Earnings</h2>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-4 md:p-6 shadow-sm">
            <h2 className="text-base md:text-lg font-bold text-black mb-4">Booking Status</h2>
            <div className="h-64 md:h-80">
              {statusData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-base md:text-xl font-bold text-black">Service Requests</h2>
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {filteredBookings.map((booking) => (
                  <div key={booking._id} className="border border-gray-100 rounded-xl p-3 hover:shadow-md transition">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h3 className="font-bold text-black text-sm">{booking.service}</h3>
                          <p className="text-xs text-gray-500">👤 {booking.userId?.name || 'Unknown'}</p>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getStatusBadge(booking.bookingStatus)}`}>
                    ...
                    {booking.workerAmount && (
                      <div className="bg-gray-50 rounded-lg p-2 mb-2 border border-gray-100">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-bold text-black text-sm">₹{booking.workerAmount}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {booking.bookingStatus === 'pending' && (
                        <>
                          <button
                            ...
                            className="flex-1 px-3 py-2 bg-black text-white rounded-lg text-xs font-semibold active:scale-95 hover:bg-gray-800 transition"
                          >
                            ✓ Accept
                          </button>
                          <button
                            ...
                            disabled={actionLoading === booking._id}
                            className="flex-1 px-3 py-2 bg-gray-200 text-black rounded-lg text-xs font-semibold active:scale-95 hover:bg-gray-300 transition"
                          >
                            ✗ Reject
                          </button>
                        </>
                      ...
                      {booking.bookingStatus === 'accepted' && (
                        <button
                          ...
                          disabled={actionLoading === booking._id}
                          className="w-full px-3 py-2 bg-black text-white rounded-lg text-xs font-semibold active:scale-95 hover:bg-gray-800 transition"
                        >
                          ✓ Mark Complete
                        </button>
                      )}
                      ...
                      {booking.bookingStatus === 'completed' && (
                        <div className="w-full text-center px-3 py-2 bg-gray-100 text-black rounded-lg text-xs font-semibold">
                          🎉 Service Completed
                        </div>
                      )}
                      
                      {booking.bookingStatus === 'rejected' && (
                        <div className="w-full text-center px-3 py-2 bg-gray-100 text-gray-600 rounded-lg text-xs font-semibold">
                          ❌ Booking Rejected
                        </div>
                      )}
                    </div>
