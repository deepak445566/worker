...
const WorkerDashboard = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // Send heartbeat
      newSocket.emit('workerOnline', { userId: user._id });
    });
    ...
    return () => {
      clearTimeout(autoRejectTimerRef.current);
      if (newSocket) {
        newSocket.emit('workerOffline', { userId: user._id });
  ...
  const handleLogout = useCallback(() => {
    if (socket) {
      socket.emit('workerOffline', { userId: user?._id });
      socket.disconnect();
    }
    logout();
    navigate('/login');
  }, [socket, logout, navigate, user]);
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">
                    👤 {notification.booking.userId?.name || 'Customer'}
                  </p>
                </div>
        ...
        <div className="bg-gray-50 border-l-4 border-black p-3 mx-3 mt-3 rounded-lg">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <p className="text-xs text-gray-700 flex-1">
              Click <strong>"Test Sound"</strong> first, then <strong>"Enable Sound"</strong> to get audio alerts for new bookings
            </p>
            <div className="flex gap-2">
              <button 
                onClick={testSound} 
                className="px-3 py-1 bg-black text-white rounded-lg text-xs font-semibold hover:bg-gray-800"
              >
                Test Sound
              </button>
              <button 
        ...
        <div className="flex justify-around items-center py-3">
          ...
          <button onClick={testSound} className="flex flex-col items-center">
            <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15.536a5 5 0 001.414 1.414m0-14.9a5 5 0 00-1.414 1.414M12 4v16" />
            </svg>
          ...
          {!soundEnabled ? (
            <button onClick={toggleSound} className="flex flex-col items-center">
              <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                  d="M5.586 15.536a5 5 0 001.414 1.414m0-14.9a5 5 0 00-1.414 1.414M12 4v16" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                  d="M17 7l5 5-5 5M7 7l-5 5 5 5" />
              </svg>
              <span className="text-xs text-black mt-1">Sound OFF</span>
            </button>
          ) : (
            <button onClick={toggleSound} className="flex flex-col items-center">
              ...
              <span className="text-xs text-green-600 mt-1">Sound ON</span>
            </button>
          )}
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              {!soundEnabled ? (
                ...
                <button 
                  onClick={toggleSound} 
                  className="px-3 py-1.5 bg-gray-200 text-black rounded-lg text-sm hover:bg-gray-300 transition"
                >
                  Disable Sound
                </button>
              )}
              ...
              <button 
                ...
                className="px-4 py-1.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition"
              >
                Logout
              </button>
            </div>
        ...
        <div className="mb-6">
          ...
          {isConnected && socketId && (
            <p className="text-xs text-green-600 mt-1">
              ✅ Real-time connection active • Ready to receive bookings
            </p>
          )}
        </div>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-1">
                <button 
                  ...
                  className={`px-2 py-1 text-xs rounded ${selectedChart === 'bar' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}
                >
                  Bar
                </button>
                <button 
                  onClick={() => setSelectedChart('line')} 
                  className={`px-2 py-1 text-xs rounded ${selectedChart === 'line' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}
                >
                  Line
                </button>
                <select 
                  ...
                  className="border border-gray-300 px-2 py-1 text-xs rounded bg-white ml-2"
                >
                  ...
                  <div key={booking._id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition">
                    ...
                    {booking.location && booking.location !== 'Address not specified' && (
                      <button
                        ...
                        className="text-black text-xs flex items-center gap-1 font-medium mb-3 hover:underline"
                      >
                    ...
                    <div className="flex gap-2">
                      {booking.bookingStatus === 'pending' && (
                        <>
                          <button
                            ...
                            className="flex-1 py-2 bg-black text-white rounded-lg text-sm font-semibold active:scale-95 hover:bg-gray-800 transition disabled:opacity-50"
                          >
                            ✓ Accept
                          </button>
                          <button
                            ...
                            disabled={actionLoading === booking._id}
                            className="flex-1 py-2 bg-gray-200 text-black rounded-lg text-sm font-semibold active:scale-95 hover:bg-gray-300 transition disabled:opacity-50"
                          >
                            ✗ Reject
                          </button>
                        </>
                      ...
                      {booking.bookingStatus === 'accepted' && (
                        <button
                          ...
                          disabled={actionLoading === booking._id}
                          className="w-full py-2 bg-black text-white rounded-lg text-sm font-semibold active:scale-95 hover:bg-gray-800 transition disabled:opacity-50"
                        >
                          ✓ Mark as Completed
                        </button>
                      )}
                      ...
                      {booking.bookingStatus === 'completed' && (
                        <div className="w-full text-center py-2 bg-gray-100 text-black rounded-lg text-sm font-semibold">
                          🎉 Service Completed - {new Date(booking.updatedAt).toLocaleDateString()}
                        </div>
                      )}
                      
                      {booking.bookingStatus === 'rejected' && (
                        <div className="w-full text-center py-2 bg-gray-100 text-gray-600 rounded-lg text-sm font-semibold">
                          ❌ Booking Rejected
                        </div>
                      )}
                    </div>
